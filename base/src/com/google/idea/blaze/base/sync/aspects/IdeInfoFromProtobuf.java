...
public class IdeInfoFromProtobuf {
  ...
  public static TargetIdeInfo makeTargetIdeInfo(IntellijIdeInfo.TargetIdeInfo message) {
    ...
    if (message.hasGoIdeInfo()) {
      goIdeInfo = makeGoIdeInfo(message.getGoIdeInfo(), key.getLabel(), kind);
  ...
  public static AndroidIdeInfo makeAndroidIdeInfo(IntellijIdeInfo.AndroidIdeInfo androidIdeInfo) {
    return new AndroidIdeInfo(
        makeAndroidResFolders(
            androidIdeInfo.getResFoldersList(), androidIdeInfo.getResourcesList()),
  ...
  private static GoIdeInfo makeGoIdeInfo(IntellijIdeInfo.GoIdeInfo info, Label label, Kind kind) {
    String libraryLabel = info.getLibraryLabel();
    if (!Strings.isNullOrEmpty(libraryLabel)) {
      label = Label.create(libraryLabel);
    }
    String libraryKind = info.getLibraryKind();
    if (!Strings.isNullOrEmpty(libraryKind)) {
      kind = Kind.fromString(libraryKind);
    }
  ...
  private static LibraryArtifact makeLibraryArtifact(
    ...
    if (!libraryArtifact.getSourceJarsList().isEmpty()) {
      sourceJars.addAll(
          libraryArtifact.getSourceJarsList().stream()
  ...
  private static List<AndroidResFolder> makeAndroidResFolders(
      List<IntellijIdeInfo.ResFolderLocation> restrictedFolders,
      List<Common.ArtifactLocation> fullFolders) {
    Set<ArtifactLocation> restrictedRoots = new HashSet<>();
    ImmutableList.Builder<AndroidResFolder> builder = ImmutableList.builder();
    for (IntellijIdeInfo.ResFolderLocation resource : restrictedFolders) {
      ArtifactLocation loc = makeArtifactLocation(resource.getRoot());
      if (loc != null) {
        restrictedRoots.add(loc);
    ...
    for (Common.ArtifactLocation pbArtifactLocation : fullFolders) {
      ArtifactLocation loc = makeArtifactLocation(pbArtifactLocation);
      // If this root was present in both "resFoldersList" and "resourcesList",
      // let the "resFoldersList" version win.
      //
      // New versions of the blaze plugin don't need this, but older versions of the plugin
      // only supported the "resourcesList" attribute, so in order to generate an ideinfo
      // that can be read by both old and new versions of the plugin, we'd need to include
      // every root in both lists.
      //
      // We can remove this branch along with the duplicate roots being generated by the
      // aspect if we no longer need to support new aspects running against an old blaze
      // plugin (for example, if we were able to bundle the aspect with the IDE rather
      // than executing it from Google3)... or if we drop support for the older plugin
      // versions that require resourcesList to be filled in.
      if (restrictedRoots.contains(loc)) {
        continue;
      }
      if (loc != null) {
        builder.add(AndroidResFolder.builder().setRoot(loc).build());
      }
    }
    return builder.build();
  }
